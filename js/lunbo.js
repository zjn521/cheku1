...
$(function(){
	new  Slideshow({
		boxId:"#box",
		imgs:["img/hero_large.jpg","img/hero_large_005.jpg","img/hero_medium_2x_002.jpg","img/hero_medium_001.jpg","img/hero_large_2x_005.jpg"],
		width:100,
		height:100,
		timeSpace:3000,
		fadeInOutTime:2000,
...
Slideshow.prototype.initUI = function(){
	...
	for(var i=0;i<this.imgs.length;i++){
		let str = "<img src='"+this.imgs[i]+"' style='width:"+this.width+"%;height:"+this.height+"%;position:absolute;left:0;top:0;z-index:"+(this.imgs.length-i)+";display:none'/>";
		$(this.boxId).append(str);
	}
	$(this.boxId+" img:first").css({"display":"block"});

	//2、所有按钮的创建
	let ulliStr = "<ul style='position:absolute;right:265px;bottom:-60px;z-index:999;display:flex;justify-content:space-between'>";
	
	for(let i=0;i<this.imgs.length;i++){
		ulliStr +="<li ord='"+(i+1)+"' style='list-style:none;float:left;margin:10px;width:"+this.btnWidth+"px;height:"+this.btnHeight+"px;text-align:center;background-color:"+this.btnColor+";'></li>"
	}
	ulliStr +="</ul>";
